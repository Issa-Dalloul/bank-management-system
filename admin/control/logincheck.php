...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $login_query = $conn->prepare("SELECT * FROM users WHERE username = ? AND password = ? AND user_type = 'admin'");
    ...
    if ($login_result->num_rows > 0) {
        ...
        // Additional admin data can be fetched if needed
        $admin_query = $conn->prepare("SELECT * FROM admin WHERE username = ?");
        ...
        if ($admin_result->num_rows > 0) {
            ...
            // Store additional admin data in the session if needed
